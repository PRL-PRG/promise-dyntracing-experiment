...
create_function_viewer_app <- function(summarized_data_dirpath,
    ...
    orders <- read_summarized_data_table("orders.csv")
                         ...
                         wellPanel(
                             fluidRow(
                                 column(4,
                                        ...
                                        selectizeInput("functionName", "Function Name", #width = "50%",
                                                       choices = nameChoices, #selected = nameChoices[1]),
                                                       options = list(placeholder = 'Please select an option below',
                                                                      maxOptions = length(nameChoices))),
                                        fluidRow(column(6, numericInput("minCallCount", "Minimum Calls", 1, min = 1, max = 100000000)),
                                                 column(6, numericInput("maxCallCount", "Maximum Calls", 100000000, min = 1, max = 100000000)))),
                                 column(8,
                                        fluidRow(column(9,checkboxGroupButtons("parameterClass",
                                                          label = "Parameter Class",
                                                          choices = list("Never" = "Never",
                                                                         "Sometimes" = "Sometimes",
                                                                         "Always" = "Always"),
                                                          selected = c("Never", "Sometimes", "Always"),
                                                          status = "primary",
                                                 ...
                                                 column(3,
                                                        tag("label",
                                                            list("Combiner",
                                                                 class = "control-label",
                                                                 `for` = "parameterClassCombiner")),
                                                        switchInput(inputId = "parameterClassCombiner",
                                                                      label = NULL,
                                                             onStatus = "success", offStatus = "danger",
                                                             onLabel = "∪", offLabel = "∩"))),
                                        fluidRow(column(9,checkboxGroupButtons("parameterMode",
                                        ...
                                        #inline = TRUE,
                                                 ...
                                                 column(3,
                                                        tag("label",
                                                            list("Combiner",
                                                                 class = "control-label",
                                                                 `for` = "parameterModeCombiner")),
                                                        switchInput(inputId = "parameterModeCombiner",
                                                                    label = NULL,
                                                                    onStatus = "success", offStatus = "danger",
                                                                    onLabel = "∪", offLabel = "∩"))),
                                        fluidRow(column(9, checkboxGroupButtons("wrapperType",
                                                                               label = "Closure Code",
                                        #inline = TRUE,
                                                                               choices = list("Wrapper" = "Wrapper",
                                                                                              "Nonwrapper" = "Nonwrapper"),
                                                                               selected = c("Wrapper", "Nonwrapper"),
                                                                               status = "primary",
                                                                               checkIcon = list(yes = icon("ok", lib = "glyphicon"),
                                                                                                no = icon("remove", lib = "glyphicon"))))))),
                             fluidRow(column(6, sliderInput("parameterRange", label = "Formal Parameter Range",
                                                            min = 0, max = 100, value = c(40, 60))),
                                      column(6, sliderInput("parameterRange", label = "Formal Parameter Range",
                                                            min = 0, max = 100, value = c(40, 60))))),
