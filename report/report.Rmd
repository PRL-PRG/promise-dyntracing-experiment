...
parameter_usage_dirpath <- path(params$output_directory, "parameters")
...
show_graph <- function(graphname) {
    include_graphics(path(visualizations, graphname))
}

show_graphs <- function(first_graphname, second_graphname) {
    load_graph <- function(graphname) {
        grid::rasterGrob(as.raster(readPNG(path(visualizations, graphname))),
                         interpolate = TRUE)
    }
