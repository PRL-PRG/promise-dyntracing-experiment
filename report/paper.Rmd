...
function_definitions <- read_table("function_definitions")
```
...
total_package_count <- length(unique(function_definitions$package))
...
package_function_distribution <-
    ...
    group_by(package) %>%
     summarize(total_function_count = length(unique(function_id))) %>%
     ungroup() %>%
...
PercAlways <-
    formal_parameter_count_by_usage_class %>% 
    filter(parameter_use=="Lookup",parameter_class=="Always") %>%
    pull(relative_parameter_count)
generate_macro("PercAlways", PercAlways)
...
argument_promise_count_by_value_lookup_count_graph <-
    ...
    mutate(value_lookup_count = replace(value_lookup_count, value_lookup_count == ">=7", "> 6")) %>%
    ...
    scale_y_continuous(sec.axis = sec_axis(~ . * total_count,labels = count_labels),
                       labels = relative_labels) +
    labs(x = "", y = "") + 
    scale_x_discrete(limits = c(0:6, "> 6"))
...
call_by_escape_and_type <- 
    ...
    filter(!(type %in% c("Unbound", "...", "Builtin", "Bytecode",
                         ...
                         "Pairlist", "Missing", "Weakref", "Promise", "Raw", "Complex")))
type_order <-
    call_by_escape_and_type %>%
...
total_package_count <- length(unique(function_definitions$package)) #nrow(package_metaprogramming)
...
expand_lifecycle <- function(action, count) {
    if(action == "()") {
        ...
        str_replace_all(str_c(rep(action_seq, count_seq), collapse = ""), "L", "R")
    }
}
...
argument_promise_call_depth_graph <-
    argument_promise_count_by_call_depth %>%
    filter(call_depth %in% c(0:5, "> 5")) %>%
    mutate(call_depth = if_else(call_depth == "0", "1",
                        if_else(call_depth == "1", "2",
                        if_else(call_depth == "2", "3",
                        if_else(call_depth == "3", "4",
                        if_else(call_depth == "4", "5",
                        if_else(call_depth == "5", "6",
                        if_else(call_depth == "> 5", "> 6", "?")))))))) %>%
    ggplot(aes(call_depth, relative_promise_count)) +
    geom_col() +
    scale_x_discrete(limits = c(1:6, "> 6")) +
    scale_y_continuous(sec.axis = sec_axis(~ . * total_count,
                                           labels = count_labels),
                       labels = relative_labels) +
    labs(x = "", y = "") + 
...
generate_macro("PercMutualForcing", CountMutualForcing / argument_promise_count_by_force_count$promise_count[2])
