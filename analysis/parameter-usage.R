...
reduce_analysis <- function(analyses) {
    ...
    analyses$`parameter-usage-count` %>%
        group_by(function_id, call_id, position) %>%
        summarize() %>%
...
summarize_analyses <- function(analyses) {
    ...
    mixed_parameter_usage_distribution <-
        analyses$`parameter-usage-count` %>%
        left_join(parameter_usage_distribution,
                  by = c("function_id" = "function_id",
                         "position" = "position")) %>%
    ...
    mixed_parameter_count <- nrow(mixed_parameter_usage_distribution)

    mixed_parameter_usage_distribution_summary <-
        mixed_parameter_usage_distribution %>%
        group_by(classification) %>%
        summarize(count = n()) %>%
        mutate(relative_count = count / mixed_parameter_count)
  ...
  ##        )
    list(force_order_distribution = force_order_distribution,
         ...
         summary = tibble(all_closure_count = all_closure_count,
                          ...
                          mixed_parameter_count = mixed_parameter_count),
         ...
         mixed_parameter_usage_distribution = mixed_parameter_usage_distribution,
         mixed_parameter_usage_distribution_summary = mixed_parameter_usage_distribution_summary)
...
visualize_analyses <- function(analyses) {
    ...
    mixed_parameter_count <- analyses$summary$mixed_parameter_count
  ...
  parameter_usage_distribution <-
    ...
    mixed_parameter_usage_distribution <-
        analyses$mixed_parameter_usage_distribution_summary %>%
        ggplot(aes(classification, weight = relative_count)) +
        geom_bar() +
        scale_y_continuous(sec.axis = sec_axis(~ . * mixed_parameter_count,
                                               labels = count_labels),
                           labels = relative_labels) +
        labs(y = "Usage",
             title =  "Mixed argument distribution by usage") +
        scale_fill_gdocs()
...
main <- function() {
    analyzer <-
        create_analyzer("Parameter Usage Analysis",
                        c("parameter-force-order", "parameter-usage-count"),
                        reduce_analysis,
                        combine_analyses,
                        summarize_analyses,
                        visualize_analyses,
