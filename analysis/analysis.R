...
read_analyses <- function(vignette_dir, analyses, logger) {

    all_files <- dir_ls(vignette_dir)
    analysis_filepaths <- path(vignette_dir, analyses)
    ...
    resolve_analysis_filepath<- function(analysis_filepath) {
        similar_filepaths <- all_files[startsWith(all_files, analysis_filepath)]
...
create_analyzer <- function(analysis_name,
    ...
    analyzer <- list(name=analysis_name,
                     analyses=analyses,
                     reduce_analysis=reduce_analysis,
...
drive_analysis <-
  function(analyzer,
    ...
    if(stage %in% c("all", "summarize")) {
        scan <- reduce_stage(analyzer, logger, settings)
