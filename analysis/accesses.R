...
analyze_database <- function(database_file_path) {
  ...
  data <- 
    ...
    mutate(
      classification = ifelse(forces > 0, ifelse(forces > 1, "multiforce", ifelse(lookups > 0, "force and reuse", "just force")), "unforced"),
      metaprogramming = ifelse(metaprogramming_any > 0, ifelse(metaprogramming_set > 0, "meta/set", "meta/get"), "clean")
    ) %>%
